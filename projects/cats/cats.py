...
def choose(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    answers = [p for p in paragraphs if select(p)]
...
def about(topic):
    ...
    def select(p):
        has_topic = False
        for word in split(remove_punctuation(lower(p))):
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    lowest_diff_word = min(valid_words, key=lambda x:diff_function(x, typed_word, limit))
    lowest_diff = diff_function(lowest_diff_word, typed_word, limit)
...
def feline_flips(start, goal, limit):
    ...
    if start == '':
        ...
        return len(start)
    elif limit == -1:   # return limit+1 once crossing the limit
        return 1
    else:
        if start[0] == goal[0]:
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    words, times = get_words(match), get_times(match)
    player_num = len(times)
    ret = [[] for _ in range(player_num)]
    for i in range(len(words)):
        ret[ min( range(player_num), key = lambda x: times[x][i]) ].append(words[i])
    return ret
    # END PROBLEM 10
