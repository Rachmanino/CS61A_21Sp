def factors_list(n):
    ...
    all_factors = []
    "*** YOUR CODE HERE ***"
    for i in range(1, n):    # search from 1 to n-1
        if n % i == 0:
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    pku = make_city('PKU', lat, lon)
    return get_name(city_a) if distance(pku, city_a) < distance(pku, city_b) else get_name(city_b)
