...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.fund < self.price:
            ...
            self.fund = 0
            self.stock -= 1
            return f'Here is your {self.good} and ${pre_fund - self.price} change.'
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        return self.cents + max(0, Mint.present_year - self.year - 50) # type: ignore
...
class VirFib():
    ...
    def next(self):
        "*** YOUR CODE HERE ***"
        nextFib = VirFib(self.value + VirFib.last_value)
        VirFib.value = self.value
        return nextFib
    last_value = 1
...
def is_bst(t):
    ...
    def bst_max(t):
        if t.is_leaf():
            return t.label
        return max(t.label, min([bst_min(branch) for branch in t.branches]))
